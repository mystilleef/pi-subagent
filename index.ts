...
async function mapWithConcurrencyLimit<TIn, TOut>(
  ...
  const workers = new Array(limit).fill(null).map(async () => {
    while (true) {
      ...
      results[current] = await fn(getArrayItem(items, current), current);
    }
  });
  ...
  pi.registerTool({
    ...
    async execute(_toolCallId, params, signal, onUpdate, ctx) {
      ...
      if (params.chain && params.chain.length > 0) {
        ...
        for (let i = 0; i < params.chain.length; i++) {
          const step = getArrayItem(params.chain, i);
      ...
      if (params.tasks && params.tasks.length > 0) {
        ...
        for (let i = 0; i < params.tasks.length; i++) {
          const task = getArrayItem(params.tasks, i);
          allResults[i] = {
            agent: task.agent,
            agentSource: "unknown",
            task: task.task,
    ...
    renderCall(args, theme, _context) {
      ...
      if (args.chain && args.chain.length > 0) {
        ...
        for (let i = 0; i < Math.min(args.chain.length, 3); i++) {
          const step = getArrayItem(args.chain, i);
    ...
    renderResult(result, { expanded }, theme, _context) {
      ...
      if (details.mode === "single" && details.results.length === 1) {
        const r = getArrayItem(details.results, 0);
        const isError =
          r.exitCode !== 0 ||
          r.stopReason === "error" ||
